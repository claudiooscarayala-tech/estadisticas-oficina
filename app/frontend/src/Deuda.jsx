...
function Deuda({ companyName }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:3001/api/deuda/upload", formData, {
        headers: {
  ...
  return (
    <div className="report-container">
      <div className="report-header">
        <h1 className="report-title">Gestión de Deuda {companyName}</h1>
      </div>
      
      <div className="card" style={{ maxWidth: "600px", margin: "2rem auto" }}>
        <h2 style={{ marginBottom: "1rem", color: "var(--text-main)" }}>Subir Archivo Excel</h2>
        <p style={{ marginBottom: "1.5rem", color: "var(--text-light)" }}>
          Sube el archivo Excel con los registros de cuotas adeudadas de {companyName}. El sistema agrupará por productor y les enviará un correo automáticamente con su archivo correspondiente.
        </p>
        ...
        >
          ...
          <input 
            ...
            id={`file-upload-${companyName.replace(/\s+/g, '-')}`}
            style={{ display: "none" }}
          />
          <label 
            htmlFor={`file-upload-${companyName.replace(/\s+/g, '-')}`}
